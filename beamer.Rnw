...
\begin{frame}[fragile]{RStudio Server Pro}
  \begin{itemize}
    \item RStudio has three parts: source code, interactive console and object explorer.
    \item Source code panel: edit the source code 
    \item interactive console: execute R code snippets
    \item object explorer: observe what objects are in memory.
  \end{itemize}
  \includegraphics[width=\textwidth]{img/rstudio_labelled.PNG}
\end{frame}
...
\begin{frame}[fragile, shrink]{Factor}

\begin{itemize}
  \item Represents categorical data
  \item Can be ordered or unordered
  \item Ordered factors can be compared
\end{itemize}
...
\subsection{List and Data Frames}
...
\subsection{Compute New Variables}
...
\begin{itemize}
  ...
  \item Can be chained with \(M\) independent variables (Multivariate)
...
\begin{description}
  \item [Linearity]
  There should be a linear relationship between the dependent  variable and independent variable(s). A linear relationship suggests that a change in Y due to one unit change in X is constant, regardless of the value of X. 
  
  \item [Distribution of error terms]
  The error terms should be normally distributed.

  \item [Variance of error terms]
...
\begin{description}
  ...
  Assesses the linearity of the relationship relationship. Look for a near horizontal line.

  \item [Normal Quantile-Quantile]
  Plots the model's residuals against a theoretical normal distribution. If the residuals lie along an approximately straight diagonal line on the   Q-Q plot, this suggests that the residuals are normally distributed.
		
  \item [Scale-Location]
  Checks for homoscedasticity - there should be no discernible pattern in the plot. Heteroscedasticity is often visible as a funnel shape and is usually caused by outliers.
  
  \item [Residual vs Leverage (Cook's Distance)]
  Identifies observations having strong influence on the model.
\end{description}
\end{frame}
...
\subsection{Multithreaded Processing}
...
\subsection{Spark MLlib (SparklyR)}
