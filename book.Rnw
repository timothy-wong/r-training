...
\item Write a new function called “square.it” that returns the square of a number.
...
\item Write a for loop that runs through a list of numbers 1:20 and prints only the even numbers. 
...
\item Write the for loop from the previous task, (runs through a list of numbers 1:20 and prints only the even numbers) as an apply function. 
\item (Advanced) Use the \verb|sys.time| function to find out if apply is faster than for.
