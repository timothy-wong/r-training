...
>>>>>>> 9a1c1d084e78bb68deabba66308b17097626afd5
...
To begin with, user can launch the default \verb|shiny| template and start editing. The code chunk in example \ref{ex:shinyplotexample} shows how the underlying model works. This is the main logic of the application and therefore should be placed in the \verb|server| component of the application.
...
Functions are located in under the \verb|R| directory. We will create a simple function called \verb|is_even()| to check whether a given interger is even or not. Users can begin by nagivating to this directory and create a new file named \verb|is_even.R| and enter the code chunk in example \ref{ex:createfunction}.
...
Once user has finished editing everything, the source code is ready to be wrapped as a package. To do this, user can navigate to \textbf{Build > Check Package}. This will check the source code and ensure the code is ready for build. Afterwards, user can click on \textbf{Build > Build Source Package}, which will wrap all the code into a \verb|.tar.gz| package file. The resulting file is a standard R package which can be transported to any user on any platform.

Another way to wrap the package is to click on \textbf{Build > Install and Restart}. This build the package and installs it directly into the user's library. In this case, the user can simply call the library and start using the custom functions rigth away.
...
The principle of reproducibility suggests that a process should always produce same results if the input remains identical. Reproducible results are crucially important for business analytics for various kind of reasons, such as compliance, quality assurance, KPI measurement... etc. The open source nature of \verb|R| and many of its packages means they get very frequent updates. There can be substantial difference between versions of the same package. 

A dependency management system called \verb|packrat| can be used to ensure reproducibility. It creates a private library for the project and searches for packages only in this location. Shared library would not be used in a \verb|packrat|-enabled project, this means the project is totally isolated and updating packages in shared library would not break the project.
...
All integer and numeric values in \verb|R| are stored as double-precision values. This means each number consumes \(64\) bits of system memory. Users should notice that significant figures has no effect on the object size. In other words, the values \(3.14\) and \(3.14159265359\) have identical object size. 
...
It is important to use data type correctly in order to conserve memory. Example \ref{ex:pryrlogicalsize} compares the object size of character vector versus logical vector.
...
The \verb|profvis()| function wraps an \verb|R| operation inside a set of curly brackets \verb|{}|, as shown in example \ref{ex:profvis}. This example generates some random numbers, runs a simple linear regression model and view the model summary.
...
Multithreading can also be used to optimise \verb|apply()| family functions and \verb|for| loops. Examples \ref{ex:parallel1} and \ref{ex:parallel2} show a simple \verb|sapply()| function can be executed using the \verb|parSapply()| function in the \verb|parallel| package. It sets up a cluster of workers which runs simuteanously, where code is executed in parallel. The cluster is stopped to release resources once the work has finished.
...
Multithreading does not always guarentee top speed. This is because it has very significant overhead due to starting and closing the threads. In addition, multithreaded processes may cause serious issue on multi-user systems, as users may compete for computational resources at the same time. Besides, using many threads simultaneously for long-running code may block other processes. This should always be used with great care.


\chapter{Distributed Computing}

Very large datasets which cannot fit in memory need to be processed differently. There are several big data tools which supports large-scale processing. The simpliest way is to carve up the large dataset into smaller chunks and process them seperately. Once completed, the overall results can be reconstructed. 

A slightly more advanced way is to process natively in a Hadoop cluster which hosts the large dataset. There are plenty of algorithms which can be executed on Hadoop, these are mainly provided by Microsoft Machine Learning Server and Apache Spark MLlib.
...
\begin{description}
  ...
  \item [Standalone] This is also known as local mode. The driver and executors run on the client machine. Although there is no distribution of computing process, prototyping and debugging in local mode is more convinient. Example \ref{ex:launchsparklocal} shows how to launch Spark locally.
...
The \verb|sparklyr| package supports Spark MLlib, which is a library for scalable machine learning algorithms. We can run a linear regression model using the package. Example \ref{ex:sparkregression} shows how to run a linear regression model on Spark.
...
Microsoft Machine Learning Server (MLS) supports distributed large scale processing on various platforms. Large dataset are stored on local file system or HDFS in \verb|XDF| file format. Data is compressed and processed as smaller chunks. 

One of the important feature of MLs is that compute context can be switched without changing the code base. Users can use the function \verb|rxSetComputeContext()| to configure where to perform the computational task, popular choices include: \verb|RxLocalSeq()|, \verb|RxLocalParallel()| and \verb|RxSpark()|.

\begin{Exercise}[title={Train a Neural Network}]

In this exercise, user will train a single-layer neural network using the \verb|flights| dataset. Example \ref{ex:mlshistogram} shows how to import data into XDF format and draw a simple histogram.
...
In example \ref{ex:mlsneuralnet}, the dataset is split into training and testing set randomly. It then fits a neural network model using the training set. Afterwards, the model is applied to training and testing set to calculate the mean-squared error (MSE) values.
