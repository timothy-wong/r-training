...
Apache Spark can be accessed in \verb|R| via the \verb|sparklyr| package, which supports the full \verb|dplyr| pipeline. Ideally, Spark can run on a YARN cluster to maximise the benefits of distributed computing. It also supports local mode which runs on a single host.

In Spark, source data can accessed through different ways. Local data in CSV format can be accessed directly. Alternatively, CSV files can be uploaded to HDFS and read from there.

The script in example \ref{} launches a Spark connection in local mode. 

It points to a user-specified database using the function tbl_change_db(). It then points to a specific table with the function tbl(). The returned object is compatible with the dplyr framework.
