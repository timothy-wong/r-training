...
A slightly more advanced way is to process it natively in a Hadoop cluster which hosts the large dataset. There are plenty of algorithms which can be executed on Hadoop, these are mainly provided by Microsoft Machine Learning Server and Apache Spark MLlib.
...
\begin{description}
  ...
  \item [Standalone] This is also known as local mode. The driver and executors run on the client machine. Although there is no distribution of computing process, prototyping and debugging in the local mode is more convenient. Example \ref{ex:launchsparklocal} shows how to launch Spark locally.
...
In Spark, source data can accessed through different ways. Files in CSV format can be accessed directly in the local file system. Alternatively, CSV files can be uploaded to HDFS and read from there. Spark also supports other formats, such as \verb|data.frame|, \verb|tibble| objects and even Hive tables in Hadoop, as data sources.

\begin{Exercise}[title={Train a Linear Regression Model}]

In this exercise, we will upload a dataset to Spark for modelling. To begin with, we will use a local \verb|tibble| as data source. 
...
The \verb|sparklyr| package supports Spark MLlib, which is a library for scalable machine learning algorithms. We can run a linear regression model using this package. Example \ref{ex:sparkregression} shows how to run a linear regression model on Spark.
...
\section{Microsoft Machine Learning Server}

Microsoft Machine Learning Server (MLS) supports distributed large scale processing on various platforms. Large datasets are stored on local file system or HDFS in \verb|XDF| file format. Data is compressed and processed as smaller chunks. 

One of the important features of MLS is that the compute context can be switched without changing the code base. Users can use the function \verb|rxSetComputeContext()| to configure where to perform the computational task. Popular choices include: \verb|RxLocalSeq()|, \verb|RxLocalParallel()| and \verb|RxSpark()|.

\begin{Exercise}[title={Train a Neural Network}]

In this exercise, users will train a single-layer neural network using the \verb|flights| dataset. Example \ref{ex:mlshistogram} shows how to import data into XDF format and draw a simple histogram.

<<ex:mlshistogram, eval=FALSE, size='scriptsize',rexample=TRUE>>=
library(nycflights13)
...
In example \ref{ex:mlsneuralnet}, the dataset is split into training and testing sets randomly. It then fits a neural network model using the training set. Afterwards, the model is applied to the training and testing sets to calculate the mean-squared error (MSE) values.

<<ex:mlsneuralnet, eval=FALSE, size='scriptsize',rexample=TRUE>>=
# Split the dataset randomly into training / testing set
mySplit <-  rxSplit(
  inData = myFlightsClean,
  outFilesBase = "mySplit",
  outFileSuffixes = c("training", "testing"),
...
myFlightsNeuralNetModel <- rxNeuralNet(
  formula = dep_delay ~ month + hour + origin + dest + carrier, 
  data = mySplit[[1]], 
  type = "regression",
  transforms = list(origin = factor(origin),
                    dest = factor(dest),
                    carrier = factor(carrier)))
# Create pointers for the training/testing prediction results
myFlighNeuralPredictionTraining <- RxXdfData("prediction_training.xdf")
myFlighNeuralPredictionTesting <- RxXdfData("prediction_testing.xdf")
# Run the model prediction
rxPredict(modelObject = myFlightsNeuralNetModel, 
          data = mySplit[[1]],
          outData = myFlighNeuralPredictionTraining,
          extraVarsToWrite = "dep_delay",
          overwrite = TRUE)
rxPredict(modelObject = myFlightsNeuralNetModel, 
          data = mySplit[[2]],
          outData = myFlighNeuralPredictionTesting,
          extraVarsToWrite = "dep_delay",
          overwrite = TRUE)
# Calculate the training and testing MSE
rxSummary(formula = ~ sq_error, 
          data = myFlighNeuralPredictionTraining,
          transforms = list(sq_error=(Score - dep_delay)^2))
rxSummary(formula = ~ sq_error, 
          data = myFlighNeuralPredictionTesting,
          transforms = list(sq_error=(Score - dep_delay)^2))
@
