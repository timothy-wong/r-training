...
In \verb|R|, looping can be slow because they are not vectorised. Moreover, \verb|R| objects are stored in memory. If the operation in the loop is altering the size of object, it will force \verb|R| to reallocate the object to a new memory address. For this reason, avoid modifying object size in loops can always speed up the code.
