...
mem_change({ myBigVec <- 1:1e6 })
# Remove a large vector
# This releases memory (-4 MB)
mem_change({ rm(myBigVec) })
@
...
Multithreaded processes may cause serious issue on multi-user systems, as users may compete for computational resources at the same time. Besides, using many threads simultaneously for long-running code may block other processes. This should always be used with great care.
