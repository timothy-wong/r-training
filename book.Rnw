...
\begin{itemize}
  \item Answers a business question (user has to define).
...
To begin with, user can modify the code chunk in example \ref{ex:shinyplotexample}. The code creates a boxplot showing time gain for each carrier.
...
myNewFlight <- tibble(date = make_date(2019,5,10),
                      ...
                      dest = "SFO", 
                      carrier="AA")
# Run the prediction - Visualise this number
